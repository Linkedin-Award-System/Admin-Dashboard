...
export const NomineeStatusChart = memo(function NomineeStatusChart({ 
  ...
  if (!data || data.length === 0) {
    return <ChartEmpty message="No nominee status data available" icon={CheckCircle} />;
  }
  ...
  const totalNominees = chartData.reduce((sum, item) => sum + item.value, 0);

  return (
    <ChartContainer title="Nominee Status" icon={CheckCircle}>
